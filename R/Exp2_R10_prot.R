...
data.file <- system.file("extdata", "Exp2_R10_prot.txt", package="DAPARdata2")
data <- read.table(data.file, header=TRUE, sep="\t", as.is=TRUE, stringsAsFactors = FALSE)

sample.file <- system.file("extdata", "samples_Exp2_R10.txt", package="DAPARdata2")
sample <- read.table(sample.file, header=TRUE, sep="\t", as.is=TRUE, stringsAsFactors = FALSE)

Exp2_R10_prot <- DAPAR2::createQFeatures(data = data, 
                                 sample = sample, 
                                 indExpData = c(1:6), 
...
saveRDS(get(Exp2_R10_prot), file=paste0(Exp2_R10_prot, '.ft'))
return(Exp2_R10_prot)
