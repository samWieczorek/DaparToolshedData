...
data.file <- system.file("extdata", "Exp2_R100_pept.txt", package="DAPARdata2")
data <- read.table(data.file, header=TRUE, sep="\t", as.is=TRUE, stringsAsFactors = FALSE)

sample.file <- system.file("extdata", "samples_Exp2_R100.txt", package="DAPARdata2")
sample <- read.table(sample.file, header=TRUE, sep="\t", as.is=TRUE, stringsAsFactors = FALSE)

Exp2_R100_pept <- DAPAR2::createQFeatures(data = data, 
                                  sample = sample, 
                                  indExpData = c(1:6), 
                                  keyId = 'Sequence', 
...
saveRDS(get(Exp2_R100_pept), file=paste0(Exp2_R100_pept, '.ft'))
return(Exp2_R100_pept)
}